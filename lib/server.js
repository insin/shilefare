...
exports.startServer = function(app) {
  ...
  timestampLog('shilefare server listening on %s', settings.baseURL)
}
...
exports.configureAllRoutes = function(app, shilefare) {
  ...
  app.post('/login', function(req, res) {
    if (req.body.user == settings.authUser && req.body.pass == settings.authPass) {
  ...
  app.post('/share', function(req, res, next) {
    ...
    fs.stat(path, function(err, stats) {
      ...
      var file = new shilefare.SharedFile(files.length, path, stats.size)
      file.generateKey(function(err) {
        if (err) return next(err)
        shilefare.files.push(file)
        res.redirect('/')
      })
    })
  })
